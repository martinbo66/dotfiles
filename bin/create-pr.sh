...
if [ $? -eq 0 ]; then
    ...
    # Extract PR URL from the output (Azure CLI typically outputs JSON with the URL)
    ...
    if [ -n "$PR_URL" ]; then
        ...
        if [ -n "$PR_ID" ]; then
            ...
            if [[ $REMOTE_URL =~ dev\.azure\.com/([^/]+)/([^/]+) ]]; then
                ...
                echo "🔗 PR Link: https://dev.azure.com/$ORG/$PROJECT/_git/$(basename -s .git $REMOTE_URL)/pullrequest/$PR_ID"
            fi
        fi
    fi
else
